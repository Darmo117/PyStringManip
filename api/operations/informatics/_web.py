...
class ToHtmlEntities(_core.Operation):
    """Convert special characters to HTML entities."""

    _SIMPLE = 'simple'
    _NON_LATIN = 'non_latin'
    _ALL = 'all'
    ...
    def __init__(self, mode: str = _NON_LATIN, type: str = _NAMED):
        """Create a to_html_entities operation.

        :param mode: Conversion mode, either 'simple' to escape only '&', '<', '>', "'" and '"',
         'non_latin' to escape all characters that are not basic latin letters, digits, spaces, tabs nor line returns,
         'all' to escape all characters.
        :param type: Entity types, either 'named', 'numeric' or 'hex'.
        """
        if mode not in (self._SIMPLE, self._NON_LATIN, self._ALL):
            raise ValueError(f'invalid mode: {mode!r}')
        if type not in (self._NAMED, self._HEX, self._NUM):
    ...
    def apply(self, s: str) -> str:
        if self._mode == self._SIMPLE:
            return html.escape(s)
        return ''.join(map(self._convert, s))

    def _convert(self, c: str) -> str:
        if self._mode == self._NON_LATIN and c.isascii() and (c.isalnum() or c in ' \n\t\r'):
            return c
        match self._type:
            case self._NAMED:
                return f'&{self._ENTITIES.get(c, f"#{ord(c)}")};'
            case self._HEX:
                return f'&#x{ord(c):x};'
            case self._NUM:
                return f'&#{ord(c)};'
...
class FromHtmlEntities(_core.Operation):
